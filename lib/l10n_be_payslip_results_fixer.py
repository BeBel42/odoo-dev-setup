...
Copied from https://github.com/romaincarlier4/odoo-dev-helper
"""
...
def parse_failures(output):
    ...
    print(
        f"{YELLOW}⚠️  Found {len(traceback_matches)} failing _validate_payslip call(s){RESET}",
    )
    print(
        f"{DIM}   ({len(actual_matches)} 'Payslip Actual Values' block(s) in output){RESET}",
    )
    ...
    for tb_match in traceback_matches:
        print(f"tb_match: {tb_match}")
        file_path = Path(tb_match.group(1).replace('/app', '/home/martin/Projects/odoo', 1))
        line_no = int(tb_match.group(2))
        method_name = tb_match.group(3)
        class_name = method_to_class.get(method_name, "TestPayslipValidation")
        file_lines = file_path.read_text(encoding='utf-8').splitlines() if file_path.exists() else []
        ...
        for i, av_match in enumerate(actual_matches):
            ...
            print(
                f"{RED}⚠️  WARNING:{RESET} could not find actual values for {file_path.name}:{line_no}",
            )
...
def apply_fixes(failures):
    ...
    for file_path, file_failures in by_file.items():
        ...
        for validate_line_no, var_name, actual_values_str in file_failures:
            ...
            if idx is None:
                print(
                    f"{RED}⚠️  WARNING:{RESET} could not find '{var_name}' assignment before {file_path.name}:{validate_line_no}",
                )
                continue
        ...
        if file_changes:
            ...
            for idx, old, new in file_changes:
                ...
                if old_dict and new_dict:
                    old_d = dict(
                        re.findall(
                            r"'(\w+(?:\.\w+)*)'\s*:\s*(-?[\d.]+)", old_dict.group(),
                        ),
                    )
                    new_d = dict(
                        re.findall(
                            r"'(\w+(?:\.\w+)*)'\s*:\s*(-?[\d.]+)", new_dict.group(),
                        ),
                    )
                    diffs = {
                        k: (old_d.get(k), new_d[k])
                        for k in new_d
                        if old_d.get(k) != new_d[k]
                    }
                    for key, (old_val, new_val) in diffs.items():
                        print(
                            f"    {DIM}{key}:{RESET} {RED}{old_val}{RESET} → {GREEN}{new_val}{RESET}",
                        )
...
def build_tags(failures):
    ...
    return ",".join(sorted({f".{method}" for *_, method in failures}))
