...
** PFA
*Précompte (professionnel) Forfaitaire d’Artiste*
C’est un régime fiscal spécifique destiné aux artistes, auteurs et interprètes qui perçoivent des revenus occasionnels ou irréguliers liés à leurs activités artistiques.
C'est pour simplifier la fiscalité pour les artistes qui ne sont pas salariés à temps plein ou qui ont des revenus variables.Aa
Un pourcentage fixe (généralement 16,5 % en 2026) est prélevé à la source sur leurs revenus artistiques, en remplacement de l’impôt progressif classique.
...
*** TODO La prime de fine d'annee elle apparaitra que sur la payslip du mois de decembre et sur une categorie differente genre "Extra Information".
- Une ONSS rule c'est une payslip rule qui doit etre payee par l'employeur mais qui apparait pas sur la payslip.
- Le truc de 12% c'est tous les mois que l'employeur dois payer mais la PFA c'est juste decembre
...
** TODOS
*** [ ] Create 1 rule that should be applied only on December to compute the net amount of the PFA. This rule should not impact GROSS --> NET of employee.
...
**** [ ] it should not appear in the payslip table but in a dedicated section
...
*** [ ] ...link that partner to the rule computing the end of year bonus in order to be able to extract the result of this rule.
*** [ ] The rule should be linked to Joint committee 302.
*** [ ] The amount is not paid by employer but by the fund (should not impact the net)
...
*** [X] That rule should be between =Accounting: Remuneration= and =Accounting: ONSS Basic (Empployer)=
*** [ ] Flexi-job and student must be excluded from the 2 rules (no monthly provision & no pfa)
    Dimona category= STU & FLX
** Talk
*** [X] Rajouter "Accounting:" devant le nom de tes rules
*** La rule des PFA doit se faire une peu comme les meal vouchers prcq les meal vouchers c'est aussi un thirt party qui fait ca.
...
* [?] [[[https://www.odoo.com/odoo/project/1251/tasks/5013546][Employee] Migrate all "general" data to demo data]]
